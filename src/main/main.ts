...
function adjustWindowPosition(name: WindowNames) {
  ...
  const windowRight = windowBounds.x + 0.15 * windowBounds.width
  const windowBottom = windowBounds.y + 0.15 * windowBounds.height
  ...
  if (windowBounds.x + 0.85 * windowBounds.width < screenBounds.x) {
    posX = screenBounds.x
  }

  if (windowBounds.y + 0.85 * windowBounds.height < screenBounds.y) {
    posY = screenBounds.y
  }
